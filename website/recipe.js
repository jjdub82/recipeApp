...
const savBtn = document.getElementById('saveFav');
...
fetch(apiUrl)
  ...
  document.getElementById('saveFav').addEventListener('click', function() {
    let favtitle = recipeList.label // replace with your title
    let favuri = recipeId;
    fetch(`/add/${favtitle}/${favuri}`)
    .then(response => response.json())
    .then(data => {
        console.log(data); // this will log the response from your server
    })
